...
for i in range(10000):
    ...
    even_sentences.append(
        " ".join([digit_to_word_map[r] for r in rand_even_ints]))
    odd_sentences.append(
        " ".join([digit_to_word_map[r] for r in rand_odd_ints]))
...
def get_sentence_batch(batch_size, data_x, data_y, data_seqlens):
    # Function to generate a batch of sentences
...
# Sequence lengths for dynamic calculation
...
with tf.variable_scope("lstm"):
    ...
    outpus, states = tf.nn.dynamic_rnn(
...
final_output = tf.matmul(
    states[1], weights["linear_layer"]) + biases["linear_layer"]
...
correct_prediction = tf.equal(
    tf.argmax(_labels, 1), tf.argmax(final_output, 1))
...
with tf.Session() as sess:
    ...
    output_example = sess.run([final_output], feed_dict={
                              _inputs: x_test, _labels: y_test, _seqlens: seqlen_test})
